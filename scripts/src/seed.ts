...
async function main() {
  ...
  const [admin] = await db
    ...
    .values([
      ...
      { name: "Maria Librarian", email: "maria@biblioteca.com", passwordHash: await bcrypt.hash("admin123", 10) },
    ])
    .returning();
  ...
  const books = await db
    .insert(booksTable)
    .values([
      { title: "The Lord of the Rings", author: "J.R.R. Tolkien", genre: "Fantasy", isbn: "978-0-618-34399-7", publishedYear: 1954, description: "An epic fantasy novel about the quest to destroy the One Ring.", status: "available" },
      ...
      { title: "To Kill a Mockingbird", author: "Harper Lee", genre: "Fiction", isbn: "978-0-06-112008-4", publishedYear: 1960, description: "A novel about racial injustice and moral growth in the American South.", status: "available" },
      { title: "The Alchemist", author: "Paulo Coelho", genre: "Fiction", isbn: "978-0-06-231500-7", publishedYear: 1988, description: "A philosophical novel about a shepherd's journey to find treasure.", status: "available" },
      ...
      { title: "The Great Gatsby", author: "F. Scott Fitzgerald", genre: "Fiction", isbn: "978-0-7432-7356-5", publishedYear: 1925, description: "A novel about the American Dream in the Roaring Twenties.", status: "available" },
      { title: "Brave New World", author: "Aldous Huxley", genre: "Science Fiction", isbn: "978-0-06-085052-4", publishedYear: 1932, description: "A dystopian novel set in a futuristic World State.", status: "available" },
      { title: "Pride and Prejudice", author: "Jane Austen", genre: "Romance", isbn: "978-0-14-143951-8", publishedYear: 1813, description: "A romantic novel of manners set in rural England.", status: "available" },
      { title: "The Hitchhiker's Guide to the Galaxy", author: "Douglas Adams", genre: "Science Fiction", isbn: "978-0-345-39180-3", publishedYear: 1979, description: "A comedic science fiction series.", status: "available" },
      { title: "Sapiens: A Brief History of Humankind", author: "Yuval Noah Harari", genre: "History", isbn: "978-0-06-231609-7", publishedYear: 2011, description: "A brief history of humankind from the Stone Age to the present.", status: "available" },
  ...
  const loans = await db
    .insert(loansTable)
    .values([
      { bookId: books[1].id, userId: readers[0].id, status: "active", loanDate: daysAgo(5), dueDate: daysFromNow(10) },
      { bookId: books[4].id, userId: readers[1].id, status: "active", loanDate: daysAgo(3), dueDate: daysFromNow(12) },
      { bookId: books[10].id, userId: readers[2].id, status: "active", loanDate: daysAgo(12), dueDate: daysFromNow(3) },
      { bookId: books[0].id, userId: readers[3].id, status: "returned", loanDate: daysAgo(30), dueDate: daysAgo(15), returnDate: daysAgo(10) },
      { bookId: books[2].id, userId: readers[0].id, status: "returned", loanDate: daysAgo(45), dueDate: daysAgo(30), returnDate: daysAgo(28) },
      { bookId: books[5].id, userId: readers[4].id, status: "overdue", loanDate: daysAgo(20), dueDate: daysAgo(5) },
      { bookId: books[9].id, userId: readers[1].id, status: "returned", loanDate: daysAgo(60), dueDate: daysAgo(45), returnDate: daysAgo(42) },
      { bookId: books[3].id, userId: readers[5].id, status: "returned", loanDate: daysAgo(25), dueDate: daysAgo(10), returnDate: daysAgo(8) },
    ])
    .returning();
  ...
  const notes = await db
    ...
    .values([
      ...
      { title: "New Books Arrived!", content: "We have received 20 new books this month! Check the catalog for the latest additions.", type: "announcement", active: true },
      { title: "Late Returns", content: "Books returned after the due date will be noted on your reader profile. Repeated late returns may result in account suspension.", type: "rule", active: true },
      { title: "Book Donation Drive", content: "The library is accepting book donations in good condition. Drop them off at the reception desk.", type: "announcement", active: false },
    ])
    .returning();
